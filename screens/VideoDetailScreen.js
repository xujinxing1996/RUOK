...
const VideoDetailScreen = () => {
  ...
  return (
    <View>
      ...
      <View style={tw`m-3`}>
        <TitleText titleText={currentVideo ? currentVideo.mediaName : '媒体名称'} />
      </View>
      ...
      <ScrollView>
        {selectedCourse.catalogueList.map((item) => (
          <View key={item.catalogueId}>
            <View style={tw`p-3 flex-row items-center`}>
              <Ionicons name="ios-map" />
              <BaseText style="ml-1">{item.catalogueName}</BaseText>
            </View>
            {item.medias &&
              item.medias.map((media) => (
                <CatalogItem key={media.mediaId} media={media} />
              ))}
          </View>
        ))}
      </ScrollView>
